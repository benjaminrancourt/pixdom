...
// IPv4 CIDR ranges that are blocked (loopback, RFC1918, link-local/cloud-metadata)
interface Cidr4 {
  base: number;
  mask: number;
}
...
async function isBlockedHost(hostname: string): Promise<boolean> {
  try {
    ...
    for (const addr of result) {
      if (addr.family === 4 && isBlockedIpv4(addr.address)) return true;
      if (addr.family === 6 && isBlockedIpv6(addr.address)) return true;
    }
  } catch {
