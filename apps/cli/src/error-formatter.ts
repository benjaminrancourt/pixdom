...
const TEMPLATES: Partial<Record<RenderErrorCode, ErrorTemplate>> = {
  ...
  INVALID_URL_PROTOCOL: {
    ...
    whatHappened: (e) => e.message,
    howToFix:
      'Only http:// and https:// URLs are supported. file://, ftp://, data://, and other protocols are not permitted.',
    docs: '--url',
    correction: null,
  },
  INVALID_URL_HOST: {
    ...
    whatHappened: (e) => e.message,
    howToFix:
      'Loopback (127.x.x.x), private (RFC1918), and cloud metadata (169.254.x.x) addresses are blocked. Use --allow-local to permit localhost rendering for development.',
  ...
  INVALID_OUTPUT_PATH: {
    ...
    whatHappened: (e) => e.message,
    howToFix:
      'Ensure the output directory exists and is writable. Paths under /dev/, /proc/, and /sys/ are not allowed.',
    docs: '--output',
    correction: null,
  },
  INVALID_FPS: {
...
function scrubSecrets(ctx: Record<string, unknown>): Record<string, unknown> {
...
function relativizePaths(msg: string): string {
...
function sanitizeFfmpegStderr(stderr: string): string {
...
export function formatError(
  ...
  if (error.cause && typeof error.cause === 'object' && !Array.isArray(error.cause)) {
    error = {
      ...error,
      cause: scrubSecrets(error.cause as Record<string, unknown>),
    };
  }

  // Relativize absolute paths in message (11.3)
  const safeMessage = relativizePaths(error.message);
  const displayError: RenderError = { ...error, message: safeMessage };
